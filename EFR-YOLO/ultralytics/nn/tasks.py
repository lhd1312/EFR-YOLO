...
DETECT_CLASS = (Detect, Detect_LSDECD)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            Classify, Conv,  GhostConv, Bottleneck,  SPP, SPPF, DWConv, Focus, BottleneckCSP,  
            DWConvTranspose2d, RGCSPELAN,}:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
        ...
        elif m in ((WorldDetect,) + DETECT_CLASS + V10_DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS):
            args.append([ch[x] for x in f])
            if m in SEGMENT_CLASS:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in (Detect_LSDECD, ):
